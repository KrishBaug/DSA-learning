...
## Smart Pointers
* A smart pointer is a c++ class that acts like a normal pointer but also manages memory automatically.
* when the smart pointer goes out of scope --> it automatically deletes the object it points to.
```cpp
void foo() {
...
1. `std::unique_ptr`
   * Owns a resource exclusively (only one pointer to it.)
...
Use case: Unique Ownership (like file handles, sockets)

2. `std::shared_ptr`:
   * Multiple smart pointers can share ownership of the same resources.
   * Uses reference counting.
   * Deletes the object only when the last `shared_ptr` is destroyed.
...
} // ✅ object freed when last owner dies
```
**Use case:** shared resources (graphs, linked structures.)

3. `std::weak_ptr`:
   * Works with `shared_ptr` but doesn`t increase reference count.
   * Used to prevent circular references (where two `shared_ptr` keep each other alive fprever).
...
struct Node {
    ...
    std::weak_ptr<Node> prev; // weak_ptr avoids cycle
};
...
* unique_ptr → lightweight wrapper around raw pointer + deletes in destructor.

* shared_ptr → maintains a hidden control block with reference count.

* weak_ptr → points to control block but doesn’t increase count.

**Rule of Thumbs :**
* Use unique_ptr by default.

* Use shared_ptr only when multiple owners are needed.
